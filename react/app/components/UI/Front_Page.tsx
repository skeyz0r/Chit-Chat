...
export default function FrontPage(props:{session:any})
{
    ...
    return(
        ...
        <LeftBar newChat={newChat} chat={chat} setMessages={setMessages} chatList={chat_list}  setList={setList} setToast={setToast} setNewChat={setNewChat} setChat={setChat} username={String(props.session.user.name)} id={Number(props.session?.user.id)}/>
       { chat.id ?
        ...
        : newChat ? <NewChat setChat={setChat} setNew={setNewChat} chat_list={chat_list} setList={setList} setToast={setToast} authorId={Number(props.session?.user.id)}/> : 
